...
function groupByKeyword(newsItems) {
    ...
    newsItems.forEach(item => {
      ...
      const addedGroups = new Set();
        ...
        // Se non corrisponde a nessuna keyword, metti in "Altro"
        const matched = keywords.some(kw => matchKeyword(searchText, kw.keyword));
        if (!matched) {
            if (!grouped['Altro']) {
                grouped['Altro'] = [];
            }
            grouped['Altro'].push(item);
        }
    });
